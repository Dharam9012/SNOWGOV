...
#from utils import charts, guis, processing
...
st.set_page_config(page_title="SNOWGOV", page_icon="snowflake.jpg",layout="wide")
...
def database_management():
    ...
    if choose == 'Schema':
        ...
        schema_team_name = st.text_input('PROJECT :', st.session_state.get('db_team_name', ''), key="schema_team_name_input")
        schema_sub_team_name = st.text_input('SUBJECT_AREA :', st.session_state.get('db_sub_team_name', ''), key="schema_sub_team_name_input")
...
def revoke_role():
    ...
    if st.button('Revoke Roles and Log'):
...
def role_assignment():
    ...
    if st.button('Grant Roles and Log'):
...
def monitor3():
    ...
    # Wrap the Plotly chart in an HTML div element with a border
    div_with_border = f'<div style="border: 2px solid black;">{fig.to_html()}</div>'

    # Display the chart in Streamlit
    st.write(div_with_border, unsafe_allow_html=True)
